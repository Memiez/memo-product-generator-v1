...
exports.handler = async (event) => {
  ...
  if (!apiKey) {
    return {
      ...
      body: JSON.stringify({ error: 'ANTHROPIC_API_KEY ยังไม่ได้ตั้งค่า — ไปที่ Netlify → Site Settings → Environment Variables แล้วเพิ่ม ANTHROPIC_API_KEY' })
    };
  }
  ...
  const { productName, brand, sku, srp } = body;
  if (!productName || !brand) {
    return { statusCode: 400, headers: CORS, body: JSON.stringify({ error: 'productName and brand are required' }) };
  }
